...
1. **选择时间范围**
   ...
   **自定义日期输入规范**：
   - 当用户选择"自定义周报"或"自定义时间段"后，必须**分两步操作**：
     1. 第一步：使用 AskUserQuestion 工具让用户选择选项（包含自定义选项）
     2. 第二步：**直接输出文本提示**（不要使用 AskUserQuestion 工具），让用户在对话中回复日期
   - **禁止**使用 AskUserQuestion 工具来收集日期输入，因为该工具的选项模式不支持自由文本输入
   ...
   **正确的交互示例**：
   ...
   第二步（直接输出文本，等待用户在对话中回复）:
   Claude 输出: "请输入周报的周一日期（格式：YYYY-MM-DD，如 2026-01-06）："
   ...
   **错误示例（禁止）**：
   ```
   # 错误1：在 AskUserQuestion 的选项中混合日期输入
   # 错误2：使用 AskUserQuestion 工具来收集日期（用户无法自由输入文本）
   ```
